...
const UserProposals = () => {
  ...
  // Installment plan fields — only shown/used for Sale + Installment
  ...
  const handleOpenPaymentModal = async (property) => {
    ...
    // Reset installment fields to defaults when opening modal
    setInstallmentMonths(360);
    setFrequency(12);
  ...
  const handleInitializePayment = async () => {
    ...
    const type = Number(selectedProperty.propertyType);
    const installment = Number(selectedProperty.isInstallment);

    // Validate installment fields before submitting
    if (type === 1 && installment === 1) {
      const months = parseInt(installmentMonths, 10);
      const freq = parseInt(frequency, 10);

      if (!months || months < 1) {
        toast.error("Please enter a valid number of installment months.");
        return;
      }
      if (!freq || freq < 1) {
      ...
      if (months % freq !== 0) {
        toast.error("Installment months must be divisible by frequency.");
        return;
      }
    }
    ...
    try {
      const { proposalId } = selectedProperty;
      ...
      if (type === 0) {
        ...
        payload.installmentMonths = parseInt(installmentMonths, 10);
      ...
      if (
        ...
        setTimeout(() => navigate("/tenant/contracts"), 1500);
      } else {
  ...
  // Is the selected proposal Sale/Installment?
  const isSaleInstallment =
    selectedProperty &&
    Number(selectedProperty.propertyType) === 1 &&
    Number(selectedProperty.isInstallment) === 1;
  ...
  if (error) {
    return (
      <div className="property-error">
        <div className="property-error-icon"><FaTimesCircle /></div>
        <h3 className="property-error-title">Oops! Something went wrong</h3>
        <p className="property-error-message">{error}</p>
        <button onClick={() => fetchMyProperties()} className="property-error-btn">
          Try Again
        </button>
      </div>
    );
              ...
              >
                ...
                <div className="user-property-header">
                  <div className="user-property-header-info">
                    <div className="user-property-icon"><FaHome /></div>
                ...
                <div className="user-property-details">
                  ...
                  <div className="user-property-detail-item">
                    ...
                    <div className="user-property-detail-content">
                      ...
                      <span className="user-property-detail-value">
                        {Number(property.isInstallment) === 1 ? "Installment" : "Cash"}
                      </span>
                    </div>
                  </div>
                  ...
                  {property.startRentalDate && property.endRentalDate && (
                    <div className="user-property-rental-period">
                      <div className="user-property-date-item">
                        <div className="user-property-date-icon start"><FaCalendarAlt /></div>
                      ...
                      <div className="user-property-date-item">
                        <div className="user-property-date-icon end"><FaCalendarAlt /></div>
                        <div className="user-property-date-content">
          ...
          <div className="card-modal-content">
            ...
            <div className="modal-body">
              {loadingCards ? (
                ...
                <>
                  {/* Installment fields — only shown for Sale/Installment */}
                  {isSaleInstallment && (
                    <div className="installment-fields">
                      <div className="installment-field-group">
                        ...
                        <span className="installment-hint">
                          Total duration of the installment plan
                        </span>
                      </div>
                      <div className="installment-field-group">
                        <label className="installment-label">
                        ...
                        />
                        <span className="installment-hint">
                          Pay every X months — must divide evenly into duration
                        </span>
                      </div>
                    </div>
                  )}
                      ...
                      >
                        ...
                        <div className="card-info">
                          <span className="card-number">
                            •••• •••• •••• {card.maskedCardNumber?.slice(-4) || card.cardNumber?.slice(-4) || "****"}
                          </span>
