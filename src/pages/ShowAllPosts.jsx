...
const InlineReplyBox = ({
  ...
  return (
    <form
      ...
      className="mt-4 p-4 bg-gray-50/80 backdrop-blur-sm border border-indigo-100 rounded-xl shadow-sm"
    >
      <textarea
        value={replyText}
        onChange={(e) => setReplyText(e.target.value)}
        placeholder="Write your reply..."
        className="w-full px-4 py-3 border border-gray-200 rounded-xl resize-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all duration-200"
        rows="3"
        autoFocus
      />{" "}
      <div className="flex items-center justify-between mt-3">
        ...
        <div>
          ...
          {!imagePreview ? (
            <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer hover:text-indigo-600 transition-colors duration-200 px-3 py-2 rounded-lg bg-white border border-gray-200 hover:border-indigo-300">
              <FaImage className="text-indigo-500" /> Add Image
              <input
                type="file"
                ref={fileInputRef}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2 text-sm bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 disabled:opacity-50 font-medium shadow-sm"
          >
...
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-md m-4 border border-gray-100"
      ...
      >
        ...
        <div className="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse rounded-b-2xl border-t border-gray-100">
          ...
          <button
            type="button"
            className="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2.5 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm transition-all duration-200"
            onClick={onClose}
          >
            Cancel
          </button>
...
const ShowAllPosts = () => {
  ...
  const updatePostCardCount = (postIdToUpdate) => {
    axios
      .get(`https://localhost:44357/api/comments/post/${postIdToUpdate}`, {
        headers: getAuthHeaders(),
      })
      .then((res) => {
  ...
  const handleSharePost = async (post, e) => {
    e.stopPropagation();
    const postUrl = `${window.location.origin}/properties/${post.postId}`;
    const shareData = {
      title: post.title,
  ...
  const handleSavePost = async (postId, e) => {
    ...
    if (wasSaved) {
      ...
      try {
        ...
        if (postToSave) {
          const imageToSend =
            postToSave.fileBase64 ||
            (postToSave.fileBase64s && postToSave.fileBase64s.length > 0
              ? postToSave.fileBase64s[0]
              : null);
          await recordHistoryEvent(userId, "save", {
            post_id: postIdStr,
            post_title: postToSave.title,
            post_image: imageToSend,
            post_price: postToSave.price,
            post_location: postToSave.location,
          });
        }
      } catch (error) {
  ...
  const handleLikePost = async (postId, e) => {
    ...
    const wasLiked =
      Array.isArray(originalPost.likes) &&
      originalPost.likes.includes(String(userId));
    const newLikesCountOptimistic = wasLiked
      ? (originalPost.likes_count || 1) - 1
      : (originalPost.likes_count || 0) + 1;
    const newLikesArrayOptimistic = wasLiked
      ? (originalPost.likes || []).filter((id) => id !== String(userId))
      : [...(originalPost.likes || []), String(userId)];
    const updatedPostOptimistic = {
      ...originalPost,
      likes_count: newLikesCountOptimistic < 0 ? 0 : newLikesCountOptimistic,
      likes: newLikesArrayOptimistic,
    };
    const updateOptimistic = (list, index) => {
      const newList = [...list];
      if (index !== -1) newList[index] = updatedPostOptimistic;
      return newList;
    };
    ...
    try {
      ...
      const finalUpdatedPost = {
        ...
        likes: liked_by_users || [],
      };
      const updateFinal = (list, index) => {
        const newList = [...list];
        if (index !== -1) newList[index] = finalUpdatedPost;
        return newList;
      };
      ...
      if (!wasLiked) {
        const imageToSend =
          originalPost.fileBase64 ||
          (originalPost.fileBase64s && originalPost.fileBase64s.length > 0
            ? originalPost.fileBase64s[0]
            : null);
        await recordHistoryEvent(userId, "like", {
          post_id: postIdStr,
          post_title: originalPost.title,
          post_image: imageToSend,
          post_price: originalPost.price,
          post_location: originalPost.location,
        });
      }
    } catch (err) {
      console.error("Like failed:", err);
      alert(err.response?.data?.error || "Failed to update like");
      const rollbackUpdate = (list, index) => {
        const newList = [...list];
        if (index !== -1) newList[index] = originalPost;
        return newList;
      };
  ...
  function CommentItem({ comment, depth = 0 }) {
    ...
    const isPostOwnerComment =
      selectedPost && String(selectedPost.userId) === String(comment.user_id);
    ...
    return (
      <div
        className={`comment-item ${comment.is_pinned ? "pinned" : ""}`}
        style={{
          ...
          <div className="comment-author">
            ...
            <div>
              ...
              <div className="comment-badges">
                {isOwner && <span className="comment-badge you">You</span>}
                {isPostOwnerComment && (
        ...
        {isEditing ? (
          ...
          <div className="comment-content">
            {comment.comment_description && (
              <p>{toPlainText(comment.comment_description)}</p>
            )}
            {originalImageUrl && (
              <img
            ...
            <div className="comments-modal-content">
              ...
              <div className="comments-modal-body">
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(251, 191, 36, 0.1), rgba(30, 58, 138, 0.1))",
                  }}
                >
                  <div className="comment-author">
                    <div className="comment-avatar">
