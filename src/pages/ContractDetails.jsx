...
const ContractDetails = () => {
  ...
  const getStatusColor = (status) => {
    ...
    if (lowerStatus === "signed" || lowerStatus === "accepted")
      return "accepted";
    ...
    return (
      <div className="contract-details-page">
        <div className="state-container">
          ...
          <p
            style={{ marginTop: "1.5rem", fontWeight: "600", color: "#64748b" }}
          >
            Loading professional contract...
          </p>
        </div>
      </div>
    );
  ...
  const handleFinalize = async () => {
    if (!paymentInfo?.externalRef) {
      toast.error("External reference not found");
      return;
    }
    if (!cvv) {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { externalRef: paymentInfo.externalRef, cvv: cvv },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response.data && response.data.success) {
        setFinalizeMessage(response.data.message || "Payment finalized successfully! 🎉");
      ...
      const errMsg = e.response?.data?.message || "Failed to finalize payment";
      setFinalizeMessage(errMsg);
      toast.error(errMsg);
    } finally {
  ...
  return (
    <div className="contract-details-page">
          ...
          <div className="header-content-wrapper">
            ...
            <div>
              <div className="header-top-row">
                ...
                <span
                  className={`status-badge ${getStatusColor(contract.status)}`}
                >
                  {contract.status}
                </span>
              </div>
        ...
        <div className="contract-body">
          ...
          <div className="contract-details-section">
            <h2 className="section-title">
              <FaMapMarkerAlt /> Property Details
            </h2>
            <div className="property-showcase-card">
              ...
              <div className="property-specs-grid">
                <div className="spec-item">
                  <FaBed /> <span>{property.numberOfRooms} Rooms</span>
                </div>
                <div className="spec-item">
                  <FaBath /> <span>{property.numberOfBathrooms} Baths</span>
                </div>
                <div className="spec-item">
                  <FaRulerCombined /> <span>{property.area} m²</span>
                </div>
                <div className="spec-item">
                  <FaLayerGroup /> <span>Floor {property.floorNumber}</span>
                </div>
                <div className="spec-item">
                  <FaChair />{" "}
                  <span>
                ...
                <div className="spec-item">
                  <FaCar />{" "}
                  <span>
                    {property.hasGarage ? "Garage Included" : "No Garage"}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="contract-details-section">
            ...
            <div className="parties-grid">
              <div className="party-card landlord">
                ...
                <div className="party-info-list">
                  <div className="party-info-item">
                    <FaEnvelope /> {parties.landlord?.email}
                  </div>
                  <div className="party-info-item">
                    <FaPhoneAlt /> {parties.landlord?.phone || "N/A"}
                  </div>
                  <div className="party-info-item">
                    <FaMapMarker /> {parties.landlord?.address || "N/A"}
                  </div>
                </div>
                <div
                  className={`signature-status-pill ${signatures.seller?.signed ? "signed" : "unsigned"}`}
                >
                  {signatures.seller?.signed ? (
                    <>
                      ...
                      {getSignedAtFor("seller") && (
                        <span
                          className="signature-date-inline"
                          style={{
                            marginLeft: "0.5rem",
                            fontSize: "0.8rem",
                            fontWeight: 600,
              ...
              <div className="party-card tenant">
                ...
                <div className="party-info-list">
                  <div className="party-info-item">
                    <FaEnvelope /> {parties.tenant?.email}
                  </div>
                  <div className="party-info-item">
                    <FaPhoneAlt /> {parties.tenant?.phone || "N/A"}
                  </div>
                  <div className="party-info-item">
                    <FaMapMarker /> {parties.tenant?.address || "N/A"}
                  </div>
                </div>
                <div
                  className={`signature-status-pill ${signatures.buyer?.signed ? "signed" : "unsigned"}`}
                >
          ...
          <div className="contract-details-section">
            <h2 className="section-title">
              <FaMoneyBillWave /> Financial Agreement
            </h2>
            <div className="details-grid">
              <div className="detail-card highlight">
                ...
                <span className="price-value">
                  {(property.price || 0).toLocaleString()} EGP
                </span>
              </div>
              ...
              {paymentPlan && (
                <>
                  <div className="detail-card">
                    ...
                    <span className="detail-value">
                      {(paymentPlan.periodicAmount || 0).toLocaleString()} EGP
                    </span>
                  </div>
                  <div className="detail-card">
                    <span className="detail-label">Duration</span>
                    <span className="detail-value">
                      {paymentPlan.durationMonths} Months
                    </span>
                  </div>
                  <div className="detail-card">
                    <span className="detail-label">Frequency</span>
                    <span className="detail-value">
                      {paymentPlan.frequencyLabel}
                    </span>
                  </div>
                  <div className="detail-card">
                    <span className="detail-label">Payments Count</span>
                    <span className="detail-value">
                      {paymentPlan.paymentsCount} Payments
                    </span>
                  </div>
                  <div className="detail-card">
                    <span className="detail-label">Platform Fee</span>
                    <span className="detail-value">
                      {paymentPlan.platformFeePercent}%
                    </span>
                  </div>
                  ...
                  {paymentPlan.startDate && (
                    <div className="detail-card">
                      ...
                      <span className="detail-value">
                        {formatDate(paymentPlan.startDate)}
                      </span>
                    </div>
                  )}
                  {paymentPlan.endDate && (
                    <div className="detail-card">
                      ...
                      <span className="detail-value">
                        {formatDate(paymentPlan.endDate)}
                      </span>
                    </div>
                  )}
                  {paymentPlan.totalAmount > 0 && (
                    <div className="detail-card">
                      ...
                      <span className="detail-value">
                        {paymentPlan.totalAmount.toLocaleString()} EGP
                      </span>
                    </div>
                  )}
                </>
            ...
            {nextActionText &&
              nextActionText.toLowerCase() !== "none" &&
              nextActionText.toLowerCase() !== "finalize" && (
          ...
          <div className="contract-footer-actions">
            ...
            {/* Finalize Payment - Only visible for Tenants AFTER the contract is fully signed */}
            {isFullySigned &&
              ...
              roleNumber === 1 && (
                ...
                >
                  <FaCreditCard />{" "}
                  {finalizeLoading ? "Finalizing..." : "Finalize Payment"}
                </button>
              )}
      ...
      {showFinalizeModal && (
        <div className="subs-modal-overlay" onClick={() => setShowFinalizeModal(false)}>
          <div className="subs-card-modal" onClick={(e) => e.stopPropagation()}>
            <div className="subs-card-modal-header">
              <h3>Finalize Payment</h3>
              <button className="subs-modal-close" onClick={() => setShowFinalizeModal(false)}>
                <FaTimes size={24} />
              </button>
            </div>
            ...
            <div className="subs-card-modal-body">
              ...
              <div className="subs-cvv-section">
                ...
                <input
                  ...
                  autoFocus
                />
                {/* <p className="cvv-hint">Enter the 3 or 4 digit security code on your card</p> */}
              </div>

              {/* <div className="finalize-ref-info">
                <span className="ref-label">External Reference:</span>
                <span className="ref-value">{paymentInfo?.externalRef}</span>
              </div> */}
            </div>

            <div className="subs-card-modal-footer">
              <button className="subs-btn-cancel" onClick={() => setShowFinalizeModal(false)}>
                Cancel
              </button>
              <button
                className="subs-btn-confirm"
                disabled={!cvv || finalizeLoading}
