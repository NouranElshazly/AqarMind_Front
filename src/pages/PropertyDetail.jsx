...
const InlineReplyBox = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await addComment(userInfo.userId, postId, commentData);
  ...
  return (
    <form
      ...
      className="mt-4 p-5 bg-bg-card rounded-2xl border border-border-primary shadow-sm"
    >
      <textarea
        value={replyText}
      ...
      <div className="flex items-center justify-between mt-4">
        <div>
          {!imagePreview ? (
            <label className="comments-upload-btn">
              <FaImage />
              <span className="comments-upload-btn-text">Add Image</span>
              <input
                type="file"
                ref={fileInputRef}
            ...
            <div className="comments-image-preview">
              <img src={imagePreview} alt="Preview" />
              <button
                type="button"
                onClick={removeSelectedImage}
                className="comments-image-preview-remove"
              >
        ...
        <div className="flex gap-3">
          ...
          >
            {isSubmitting ? (
              <>
                <div className="comments-submit-spinner"></div>
                Replying...
              </>
            ) : (
              <>
...
const PropertyDetail = () => {
  ...
  useEffect(() => {
    if (showComments) {
      document.body.classList.add("modal-open-hide-elements");
    } else {
      document.body.classList.remove("modal-open-hide-elements");
    }
    return () => {
      document.body.classList.remove("modal-open-hide-elements");
    };
  ...
  const handleSubmitComment = async (e) => {
    ...
    try {
      ...
      const response = await addComment(userId, postId, commentData);
  ...
  const handleEditComment = async (commentId, updatePayload) => {
    try {
      ...
      const response = await updateComment(commentId, finalPayload);
  ...
  function CommentItem({ comment, depth = 0 }) {
    ...
    return (
      <div
        className={`comment-item-card ${comment.is_pinned ? "pinned" : ""} ${depth > 0 ? "ml-8 mt-4" : ""}`}
      >
        <div className="comment-item-header">
          <div className="comment-author-info">
            ...
            <div className="comment-author-details">
              ...
              <div className="comment-badges">
                {isOwner && (
                  <span className="comment-badge comment-badge-you">You</span>
                )}
                {isPostOwnerComment && (
                  <span className="comment-badge comment-badge-owner">
                    Owner
                  </span>
                )}
                {comment.is_pinned && (
                  <span className="comment-badge comment-badge-pinned">
                    Pinned
                  </span>
                )}
              </div>
          ...
          <div className="text-right">
            ...
            {comment.is_edited && (
              <span className="comment-badge-edited">
          ...
          <div className="comment-actions">
            ...
            {isCurrentUserPostOwner && (
              <button
                ...
                disabled={isProcessing}
                className={`comment-action-btn pin-btn ${comment.is_pinned ? "pinned-btn" : ""}`}
              >
                {loadingAction === `pinning-${comment._id}` ? (
          ...
          <div className="comments-modal-container">
            <div className="comments-modal-header">
              <h2 className="comments-modal-title">
                <div className="comments-modal-title-icon">
                  <FaComment />
                </div>
            ...
            <div className="comments-modal-body">
              {userInfo && (
                <div className="comments-user-badge">
                  <p className="comments-user-badge-text">
                    Commenting as:{" "}
                    <span className="comments-user-badge-name">{userName}</span>
                    {post && String(post.userId) === String(userId) && (
                      <span className="comments-user-badge-owner">
                        <FaUserTie />
                        Owner
                      </span>
                    )}
              ...
              <form onSubmit={handleSubmitComment} className="mb-8">
                {replyingToId && (
                  <div className="comments-reply-banner">
                    <p className="comments-reply-banner-text">
                      Replying to{" "}
                      <strong>
                        {
                          comments
                    ...
                    <button
                      ...
                      className="comments-reply-banner-close"
                    >
                      <FaTimes />
                    </button>
                  </div>
                ...
                <div className="comments-form-actions">
                  <div>
                    {!imagePreview ? (
                      <label className="comments-upload-btn">
                        <FaImage />
                        <span className="comments-upload-btn-text">
                          Add Image
                        </span>
                        <input
                          type="file"
              ...
              <div className="comments-list">
                {loadingAction === "loading-comments" ? (
                  ...
                  <div className="comments-empty">
                    <FaComment className="comments-empty-icon" />
